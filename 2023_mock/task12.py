...
class Puzzle():
	...
	def SwampThisCell(self):
		EmptyCells = [i for i in self.__Grid if i.GetSymbol() == "-"]
		for i in range(self._NumberSwamps):
			if len(EmptyCells):
				Cell = random.randint(0, len(EmptyCells) - 1)
				self.__Grid[Cell] = BlockedCell()
				self.__Grid[Cell].UpdateCell()
				EmptyCells.pop(Cell)
			else:
				break  # Could be possible for no empty cells remaining, so would throw errors.
	...
	def AttemptPuzzle(self):
		Finished = False
		SwampTriggeredTurns = None  # Counter for number of turns until swamps. None if not determined yet. -1 if triggered
		self._NumberSwamps = random.randint(1, 4)   # Can be done here, as swamps are only triggered once.
		while not Finished:
			if SwampTriggeredTurns is None and SwampTriggeredTurns != -1 and random.random() <= 1:
				SwampTriggeredTurns = random.randint(2, 4)

			if SwampTriggeredTurns == 0:
				print("\nSwamps have been triggered")
				self.SwampThisCell()
				SwampTriggeredTurns = -1
			self.DisplayPuzzle()
			print("Current score: " + str(self.__Score))
			if SwampTriggeredTurns is not None and SwampTriggeredTurns > 0:
				print(f"Swamps triggered in {SwampTriggeredTurns} turns. There will be {self._NumberSwamps} placed.")
			...
			if self.__SymbolsLeft == 0:
				Finished = True
			elif SwampTriggeredTurns is not None and SwampTriggeredTurns != -1:
				SwampTriggeredTurns -= 1
		print()
		self.DisplayPuzzle()
		print()
