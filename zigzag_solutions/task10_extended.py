# This is an extended version of task 10. In the question, only errors relating to adding a symbol to the board need to
# fixed. However, the question focuses on handling potential errors in the program. It contains catch-all exception
# handling which is bad practice, and can prevent the program from being exited easily. With some of these, the program
# does not give the user an explanation, or justification for what has happened, which can be confusing. With others, it
# can completely stop the program for working.
#
# Consequently, this extended task 10 completes all the criteria for task 10, and also addresses issues with using
# invalid symbols (no error message), catch-all exceptions preventing the program from being stopped, and loading games
        ...
        while not Valid:
            ...
            if len(Filename) > 0:
                try:
                    ...
                    print(f"Game, '{Filename}', does not exist. Please enter a valid name, or use the standard puzzle.")
            else:
...
########################################################################################################################
            print("Remaining turns:", str(self.__SymbolsLeft))
            ValidMove = False
            while not ValidMove:
                Row = ""
                Valid = False
                while not Valid:
                    ...
                    try:
                        ...
                        print(f"Row '{Row}' isn't valid. Please enter a value between 1 and {self.__GridSize} inclusive.")
                Column = ""
                Valid = False
                while not Valid:
                ...
                if CurrentCell.CheckSymbolAllowed(Symbol) and not CurrentCell.GetPartOfPattern():
                    ...
                    print(f"Cell at position ({Row}, {Column}) is blocked cell. This move is not valid.")
                elif CurrentCell.GetPartOfPattern():
                    print(f"Cell at position ({Row}, {Column}) is part of a pattern. This move is not valid.")
                else:
                    print(f"Symbol {Symbol} cannot be added to the cell at position ({Row}, {Column}). This move is not valid.")
...
########################################################################################################################
        Symbol = ""
        Valid = False
        ValidSymbolString = ", ".join(self.__AllowedSymbols[:-1]) + " or " + self.__AllowedSymbols[-1]
        while not Valid:
            Symbol = input("Enter symbol: ")
            if Symbol in self.__AllowedSymbols:
                Valid = True
            else:
                print(f"Symbol, '{Symbol}', is not a valid symbol. Please enter a valid symbol ({ValidSymbolString}).")
        return Symbol
########################################################################################################################
...
########################################################################################################################
        if self._Symbol == SymbolToAdd and SymbolToAdd != "":
            self.AddToPattern()
########################################################################################################################
...
########################################################################################################################
    def GetPartOfPattern(self):
        return self.__InPattern

    def AddToPattern(self):
