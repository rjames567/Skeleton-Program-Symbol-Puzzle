# The mark scheme for question 14 works correctly with the default puzzles. However, there are circumstances where this
# will not work correctly. This file follows the approach expected by the mark scheme.
...
class Puzzle():
    ...
    def TestForError1(self):
        ...
        for C in self.__Grid:
            if len(C.GetNotAllowedSymbols()) and C.GetSymbol() != "@":
                Found = True
        return Found and not self.__Score

    def TestForError2(self):
        Count = 0
        for C in self.__Grid:
            if len(C.GetNotAllowedSymbols()) or C.GetSymbol() == "@":
                Count += 1
        return (self.__Score / 10) * 9 > Count
        ...
        try:
            with open(Filename) as f:
                ...
                if self.TestForError1():
                    print("Error 1 is present in the game file")
                if self.TestForError2():
                    print("Error 2 is present in the game file")
...
class Cell():
    ...
    def GetNotAllowedSymbols(self):
        return [i for i in self.__SymbolsNotAllowed if i != ""]
