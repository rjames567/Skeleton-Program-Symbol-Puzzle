...
########################################################################################################################
                        RotationMatch = self.MatchesPatternRotated(
                            P.GetPatternSequence(),
                            PatternString, CurrentSymbol,
                            P.GetSymbol()
                        )
                        if P.MatchesPattern(PatternString, CurrentSymbol) or RotationMatch:
########################################################################################################################
...
########################################################################################################################
                            return 15 if RotationMatch else 10
########################################################################################################################
                except:
                    pass
        return 0
...
########################################################################################################################
    def MatchesPatternRotated(self, TargetPattern, CheckPattern, Symbol, TargetSymbol):
        Prev = TargetPattern
        RotatedPatterns = []
        for Rotation in range(3):
            Prev = Prev[6:8] + Prev[:6] + Prev[8]
            RotatedPatterns.append(Pattern(TargetSymbol, Prev))

        for P in RotatedPatterns:
            if P.MatchesPattern(CheckPattern, Symbol):
                return True
        return False
########################################################################################################################
