    ...
    def __SavePuzzle(self):
        ...
        while not Available:
            ...
            try:
                with open(Filename + ".txt", "r"):
                    Available = input(f"{Filename} already exists. Do you want to overwrite it (y/n): ").lower() == "y"
        ...
        with open(Filename + ".txt", "w+") as f:
            ...
            for P in self.__AllowedPatterns:
                f.write(P.GetSymbol() + "," + P.GetPatternSequence() + "\n")
            f.write(str(self.__GridSize) + "\n")
            for Count in range(self.__GridSize * self.__GridSize):
                Cell = self.__Grid[Count]
                Symbol = Cell.GetSymbol()
                if Symbol == "-":
                    Symbol = ""
                f.write(Symbol + "," + ",".join(Cell.GetNotAllowedSymbols()) + "\n")
...
########################################################################################################################
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            self.DisplayPuzzle()
########################################################################################################################
            save = input("Save and exit the game (y/n): ").lower() == "y"
            if save:
                self.__SavePuzzle()
                Finished = True
            else:
