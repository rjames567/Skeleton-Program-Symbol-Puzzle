# This is an extended version of task 7. It is given in the question that the score changes do not need to be taken into
# consideration. This extended version of the task does change the score if necessary
...
########################################################################################################################
            ...
            if len(self.__MoveStack):
                undo = input("Do you want to undo your last move (y/n)? ").lower() == "y"
            else:
                undo = False
            if undo:
                self.UndoPreviousMove()
                ...
                if CurrentCell.CheckSymbolAllowed(Symbol):
                    self.__MoveStack.append(PreviousMove(Row, Column, CurrentCell, self.__Score))
...
########################################################################################################################
    def UndoPreviousMove(self):
        replace = self.__MoveStack.pop()
        self.__Grid[(self.__GridSize - replace.GetRow()) * self.__GridSize + replace.GetColumn() - 1] = replace.CreateOldCell()
        self.__Score = replace.GetPreviousScore()
        self.__Score = replace.GetPreviousScore()
########################################################################################################################
...
class PreviousMove(Cell):
    def __init__(self, Row, Col, PrevCell, PrevScore):
        super().__init__()
        self.__Row = Row
        self.__Col = Col
        symbol = PrevCell.GetSymbol()
        self.ChangeSymbolInCell("" if symbol == "-" else symbol)
        self.__SymbolsNotAllowed = PrevCell.GetNotAllowedSymbols()
        self.__PrevScore = PrevScore
    ...
    def CreateOldCell(self):
        NewCell = Cell()
        NewCell.ChangeSymbolInCell(self._Symbol)
        for i in self.__SymbolsNotAllowed:
            NewCell.AddToNotAllowedSymbols(i)
        return NewCell
