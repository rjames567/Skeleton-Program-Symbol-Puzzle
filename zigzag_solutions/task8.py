...
########################################################################################################################
    def ReShuffleBlockedCells(self):
        EmptyCells = []
        Count = 0
        for Index, CellContents in enumerate(self.__Grid):
            Symbol = CellContents.GetSymbol()
            if Symbol == "@":
        ...
        Locations = random.shuffle(EmptyCells)[:Count]
...
########################################################################################################################
                    Shuffle = input("Do you want to redistributed the blocked cells (y/n)?").lower() == "y"
                    if Shuffle:
